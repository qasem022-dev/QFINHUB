"use client";

import * as React from "react";
import { CalculatorLayout, CalculatorInput, CalculatorChart, ResultCard } from "..";
import { formatCurrency, formatNumber } from "@/lib/utils";

export default function DebtPayoffCalculator() {
  const [totalDebt, setTotalDebt] = React.useState(15000);
  const [rate, setRate] = React.useState(18);
  const [monthlyPayment, setMonthlyPayment] = React.useState(500);
  const [extraPayment, setExtraPayment] = React.useState(0);

  const mr = rate / 100 / 12;
  const totalMonthly = monthlyPayment + extraPayment;
  const minPayment = totalDebt * mr;

  // Calculate payoff with minimum
  let balanceMin = totalDebt;
  let monthsMin = 0;
  let totalInterestMin = 0;
  const minMonthly = Math.max(monthlyPayment, minPayment);

  const chartData: { month: string; "With Extra Payment": number; "Without Extra": number }[] = [
    { month: "Start", "With Extra Payment": Math.round(totalDebt), "Without Extra": Math.round(totalDebt) },
  ];

  // With extra payment
  let balanceExtra = totalDebt;
  let monthsExtra = 0;
  let totalInterestExtra = 0;
  while (balanceExtra > 0 && monthsExtra < 600) {
    const interest = balanceExtra * mr;
    const principalPmt = Math.min(totalMonthly - interest, balanceExtra);
    totalInterestExtra += interest;
    balanceExtra -= principalPmt;
    monthsExtra++;
    if (monthsExtra % 6 === 0 || balanceExtra <= 0) {
      chartData.push({
        month: `Mo ${monthsExtra}`,
        "With Extra Payment": Math.round(Math.max(0, balanceExtra)),
        "Without Extra": 0,
      });
    }
  }

  // Without extra payment
  let balanceNoExtra = totalDebt;
  let monthsNoExtra = 0;
  let totalInterestNoExtra = 0;
  while (balanceNoExtra > 0 && monthsNoExtra < 600) {
    const interest = balanceNoExtra * mr;
    const principalPmt = Math.min(monthlyPayment - interest, balanceNoExtra);
    totalInterestNoExtra += interest;
    balanceNoExtra -= principalPmt;
    monthsNoExtra++;
    if (monthsNoExtra % 6 === 0 || balanceNoExtra <= 0) {
      const idx = chartData.findIndex(d => d.month === `Mo ${monthsNoExtra}`);
      if (idx >= 0) {
        chartData[idx]!["Without Extra"] = Math.round(Math.max(0, balanceNoExtra));
      } else {
        chartData.push({
          month: `Mo ${monthsNoExtra}`,
          "With Extra Payment": 0,
          "Without Extra": Math.round(Math.max(0, balanceNoExtra)),
        });
      }
    }
  }

  const savings = totalInterestNoExtra - totalInterestExtra;

  return (
    <CalculatorLayout
      title="Debt Payoff Calculator"
      description="Calculate how long it will take to pay off debt with various monthly payment strategies."
      icon={<span>💪</span>}
      results={
        <div className="space-y-4">
          <ResultCard label="Months to Payoff" value={`${monthsExtra} months (${(monthsExtra / 12).toFixed(1)} yrs)`} highlight />
          <ResultCard label="Total Interest" value={formatCurrency(totalInterestExtra)} />
          <ResultCard label="Interest Savings" value={formatCurrency(savings)} subtext={`vs. $${formatCurrency(monthlyPayment)}/mo minimum`} />
          <ResultCard label="Standard Payoff" value={`${monthsNoExtra} months (${(monthsNoExtra / 12).toFixed(1)} yrs)`} subtext={`At ${formatCurrency(monthlyPayment)}/mo`} />
        </div>
      }
    >
      <CalculatorChart type="line" data={chartData} xKey="month" yKey={["With Extra Payment", "Without Extra"]} title="Debt Balance Over Time" />
      <CalculatorInput input={{ id: "totalDebt", label: "Total Debt", type: "number", defaultValue: 15000, suffix: "$", min: 0 }} value={totalDebt} onChange={setTotalDebt} />
      <CalculatorInput input={{ id: "rate", label: "Annual Interest Rate", type: "number", defaultValue: 18, suffix: "%", min: 0, max: 100, step: 0.1 }} value={rate} onChange={setRate} />
      <CalculatorInput input={{ id: "monthlyPayment", label: "Monthly Payment", type: "number", defaultValue: 500, suffix: "$", min: 0 }} value={monthlyPayment} onChange={setMonthlyPayment} />
      <CalculatorInput input={{ id: "extraPayment", label: "Extra Monthly Payment", type: "number", defaultValue: 0, suffix: "$", min: 0 }} value={extraPayment} onChange={setExtraPayment} />
    </CalculatorLayout>
  );
}
